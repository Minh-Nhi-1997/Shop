...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $role = $_POST['role'] ?? 'customer'; // Lấy role từ form, mặc định customer

    if ($action === 'add') {
        $hashedPassword = password_hash($password, PASSWORD_DEFAULT);
        $stmt = $conn->prepare("
            INSERT INTO customers (full_name, email, password_hash, phone, address, role, created_at) 
            VALUES (?, ?, ?, ?, ?, ?, NOW())
        ");
        $stmt->bind_param("ssssss", $full_name, $email, $hashedPassword, $phone, $address, $role);
        $stmt->execute();
        $stmt->close();
    ...
    if ($action === 'edit') {
        if (!empty($password)) {
            ...
            $stmt = $conn->prepare("
                UPDATE customers 
                SET full_name=?, email=?, password_hash=?, phone=?, address=?, role=? 
                WHERE customer_id=?
            ");
            $stmt->bind_param("ssssssi", $full_name, $email, $hashedPassword, $phone, $address, $role, $id);
        } else {
            $stmt = $conn->prepare("
                UPDATE customers 
                ...
                WHERE customer_id=?
            ");
            $stmt->bind_param("sssssi", $full_name, $email, $phone, $address, $role, $id);
        }
        $stmt->execute();
        $stmt->close();
...
<body>
            ...
            <form class="modal-content" method="post" id="customerForm">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="role" id="customerRole" class="form-select" required>
    ...
    <script>
        ...
        $('.edit-btn').on('click', function() {
            ...
            $('#customerRole').val(btn.data('role')); // Set role hiện tại
